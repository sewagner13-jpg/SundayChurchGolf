...
Deployment is authorized only through the attended production gate. After the exact commit is server-confirmed, update only production draft `cmsuqvycj0001la09ri2bq1fr` to Sunday Church Yellow Ball Skins with handicaps enabled. Preserve its selected players, generated teams, course, date, visibility, and $30 buy-in. Do not create a disposable smoke round.
