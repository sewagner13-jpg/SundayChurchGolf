...
- Plus handicaps remain supported: the lowest rounded handicap still becomes zero and every other player retains the rounded difference from that baseline.
...
- Mark each cell with `1`, `2`, or the applicable stroke count and show a dash when no stroke applies.
